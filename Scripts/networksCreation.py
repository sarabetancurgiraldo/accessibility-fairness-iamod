# -*- coding: utf-8 -*-
"""
Created on Wed Apr  3 10:21:37 2024
...
def shrink_graphs(target, G_b, G_w, G_c):
    shrink_G_b = Shrink(G_b)
    shrink_G_w = Shrink(G_w)
    shrink_G_c = Shrink(G_c)
    
    #Execute Shrink to get 1000 nodes graph
    executions_b = len(G_b) - target
    executions_w = len(G_w) - target
    executions_c = len(G_c) - target
    
    G_b = shrink_G_b.Execute(executions_b)
    G_w = shrink_G_w.Execute(executions_w)
    G_c = shrink_G_c.Execute(executions_c)
