function plot_modal_share(T_max,accType,fp_load,fp_save,fp_save_fig,Tavg, ...
                          G,D,maxY,X)
...
figure('Position',3*[0 0 192 (2/3)*144]);
hold on;
grid on;
box on;
...
% y_max = ceil(max(sum(T,2))/1e3) * 1e3;
ylim([0 maxY])
% if abs(round(T_max*60)/2 - round(T_max*60/2)) < eps(1)
%     ylim([0 3000])
% else
%     ylim([0 8000])
% end
