function maxPathAccMILP(G,B,pc_unique,Xfast,Xslow,D,nOD,R_selector,Nmin, ...
...
tfast           = t'*Xfast;
tslow           = t'*Xslow;
Efast           = (max(0, tfast - Tmax)/Tmax);
Eslow           = (max(0, tslow - Tmax)/Tmax);
...
Cons            = [Bcar*(sum(X(arcsCar,:),2)+xR)                == 0;
                   ...
                   Ffast                                        >= 0;
                   Fslow                                        >= 0
                   (Ffast.*Efast+Fslow.*Eslow)./alpha - Tmax    <= (1-b)*M;
                   epsilon                                      >= Nmin - N;
                   epsilon                                      >= 0]; 
