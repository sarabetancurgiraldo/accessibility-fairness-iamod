...
deltaN_OD_AvgAcc = population_region'*dest_def_OD_AvgAcc/sum(population_region);
b_path = zeros(nOD,1); b_path(find(~AFI)) = 1; 
dest_def_path_AvgAcc = max(0,Nmin-R_selector*b_path);
deltaN_path_AvgAcc = population_region'*dest_def_path_AvgAcc/sum(population_region);

% Path-Acc DestDeficit
load("output/AFI_heatmap_pathAcc.mat");
b_OD = zeros(nOD,1); b_OD(find(~AFI_epsilons)) = 1; 
dest_def_OD_pathAcc = max(0,Nmin-R_selector*b_OD);
deltaN_OD_pAcc = population_region'*dest_def_OD_pathAcc/sum(population_region);
b_path = zeros(nOD,1); b_path(find(~AFI)) = 1; 
dest_def_path_pathAcc = max(0,Nmin-R_selector*b_path);
deltaN_path_pAcc = population_region'*dest_def_path_pathAcc/sum(population_region);
...
plot_modal_share_dif(T_max, AvgAcc, destAcc, fp_save, 'Average')

% PathAcc vs Dest 
fp_save = sprintf('output/figures/modal_share_dif_OD_PathDest.%s',file_typ);
plot_modal_share_dif(T_max, pathAcc, destAcc, fp_save, 'Average')
...
plot_modal_share_dif(T_max, AvgAcc, destAcc, fp_save, 'Path')

% PathAcc vs dest 
fp_save = sprintf('output/figures/modal_share_dif_path_PathDest.%s',file_typ);
plot_modal_share_dif(T_max, pathAcc, destAcc, fp_save, 'Path')
